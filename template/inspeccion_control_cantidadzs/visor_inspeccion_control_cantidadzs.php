...
$ruta_json = "../../archivos/generados/inspeccion_control_cantidadzs/" . $sede_dir . "/" . basename($target_file);
...
$periodo = str_replace(['INGCANTIDAD_', '.json'], '', basename($target_file));

$items_labels = [
    'eval_si_unidades'          => 'Las cantidades de productos preempacados se declaran con las unidades del SI',
    'eval_etiquetado'           => 'Los productos cumplen los requisitos de etiquetado, declaración de cantidad y presentación de la información',
    'eval_req_metrologicos'     => 'Los requisitos metrológicos para preempacados están debidamente identificados y documentados en el procedimiento',
    'eval_deficiencias'         => 'Las deficiencias tolerables de los productos de la organización están especificadas en el procedimiento',
    'eval_muestreos_reg'        => 'Se ejecutan y registran los muestreos de los productos para su respectivo control de cantidad',
    'eval_proc_muestreos'       => 'Se ejecutan procedimientos para examinar los muestreos realizados a los productos en el respectivo control de cantidad',
    'eval_tara'                 => 'Se determina y se usa la tara para determinar el contenido real de los productos preempacados',
    'eval_cantidad_no_regulada' => 'El producto cuya cantidad no está regulada por la legislación vigente cumple con los requisitos del cliente y se lleva un registro de su control',
    'eval_basculas'             => 'Se cuentan con procedimientos y planes para la comprobación y mantenimiento de las básculas e instrumentos de medición de peso/masa',
    'eval_no_enganosos'         => 'Se garantiza que los productos preempacados no sean engañosos para el cliente en cuanto a su declaración de cantidad',
];
...
<head>
    ...
    <title>Visor — Inspección Control de Cantidad | <?= htmlspecialchars($periodo) ?></title>
...
<div class="action-bar">
    <a href="rev_inspeccion_control_cantidadzs.php" class="btn btn-back">
        ← Volver a Galería
    </a>
...
<div id="documento">
    ...
    <div class="doc-header">
        ...
        <div class="doc-header-center">
            ...
            <div class="doc-title">Procedimiento Control de Cantidad</div>
            <div class="doc-subtitle">"Inspección Procedimiento Control de Cantidad"</div>
        </div>
        <div class="doc-header-meta">
            <div class="meta-row">
                <div class="meta-key">Código:</div>
                <div class="meta-val-cell">GP-PD-FP-GP-FO-012</div>
            </div>
            <div class="meta-row">
            ...
            <div class="meta-row">
                ...
                <div class="meta-val-cell">1/06/2021</div>
            </div>
            <div class="meta-row">
    ...
    <table class="tabla-principal">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?= fmtEval($d['eval_si_unidades']          ?? '—') ?></td>
                <td><?= fmtEval($d['eval_etiquetado']           ?? '—') ?></td>
                <td><?= fmtEval($d['eval_req_metrologicos']     ?? '—') ?></td>
                <td><?= fmtEval($d['eval_deficiencias']         ?? '—') ?></td>
                <td><?= fmtEval($d['eval_muestreos_reg']        ?? '—') ?></td>
                <td><?= fmtEval($d['eval_proc_muestreos']       ?? '—') ?></td>
                <td><?= fmtEval($d['eval_tara']                 ?? '—') ?></td>
                <td><?= fmtEval($d['eval_cantidad_no_regulada'] ?? '—') ?></td>
                <td><?= fmtEval($d['eval_basculas']             ?? '—') ?></td>
                <td><?= fmtEval($d['eval_no_enganosos']         ?? '—') ?></td>
    ...
    <div class="doc-footer">
        ...
        <span>Generado: <?= date('d/m/Y H:i') ?> &nbsp;|&nbsp; GP-PD-FP-GP-FO-012 v1</span>
    </div>
...
<script>
    async function exportarPDF() {
        ...
        try {
            ...
            pdf.save('Inspeccion_ControlCantidad_<?= preg_replace('/[^A-Za-z0-9_-]/', '', $periodo) ?>_<?= htmlspecialchars($sede) ?>.pdf');
