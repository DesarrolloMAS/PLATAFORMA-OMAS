...
if (!$input_array || empty($input_array['harina'])) {
    http_response_code(400);
    echo json_encode(['status' => 'error', 'message' => 'Faltan datos requeridos (harina).']);
    exit;
}

$nuevo_registro = [
    'id_registro' => uniqid('FAMILIAR_'),
...
// Un archivo JSON por producto (sin rotación mensual)
$harina_saneada = preg_replace('/[^A-Za-z0-9._-]/', '_', $input_array['harina']);
$base_dir       = "../../archivos/generados/cantidad_familiar/";
$sede_dir       = $base_dir . preg_replace('/[^A-Za-z0-9_-]/', '', $sede) . "/";
$archivo_json   = $sede_dir . $harina_saneada . ".json";
