...
$base_dir    = "../../archivos/generados/inspeccion_almacenamiento/";
$sede_dir    = $base_dir . preg_replace('/[^A-Za-z0-9_-]/', '', $sede) . "/";
$archivo_json = $sede_dir . "INSALM_" . $mes_actual . ".json";
...
if (@file_put_contents($archivo_json, json_encode($datos_existentes, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE))) {
    echo json_encode([
        'status'  => 'success',
        'message' => 'Inspección de almacenamiento registrada correctamente.',
        'id'      => $nuevo_registro['id_registro']
    ]);
} else {
    $err    = error_get_last();
    $errMsg = $err ? $err['message'] : 'No se pudo escribir en el archivo JSON (revisar permisos).';
