...
if (file_exists($target_dir)) {
    foreach (scandir($target_dir) as $file) {
        ...
        $last_pct     = isset($ultimo['datos']['porcentaje_cumplimiento'])
...
$total_reg_global  = array_sum(array_column($archivos, 'registros'));
...
<head>
    ...
    <title>Revisiones - Inspección PPR Almacenamiento</title>
    ...
    <style>
        :root {
            ...
            --danger: #FF3366; --danger-glow: rgba(255, 51, 102, 0.4);
            --success: #10B981; --warning: #FFB000;
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header-box {
            ...
            border-left: 4px solid var(--accent); padding: 30px; border-radius: var(--r-md);
        ...
        .stat-card {
            ...
            display: flex; flex-direction: column; gap: 4px; transition: border-color 0.3s;
        }
        ...
        .btn-new {
            ...
            white-space: nowrap; transition: all 0.3s; box-shadow: 0 0 15px var(--accent-glow);
        ...
        .file-card {
            background: var(--panel-bg); border: 1px solid var(--border-color);
            border-radius: var(--r-md); padding: 25px; display: flex;
            flex-direction: column; gap: 15px; transition: all 0.3s ease;
        ...
        .periodo-badge {
            ...
            border: 1px solid rgba(0,240,255,0.2); padding: 4px 10px;
            border-radius: var(--r-sm); font-family: 'Space Mono', monospace; font-size: 11px; font-weight: 700;
        }
        .cumplimiento-badge {
            padding: 4px 10px; border-radius: var(--r-sm);
            font-family: 'Space Mono', monospace; font-size: 12px; font-weight: 700; white-space: nowrap;
        }
        ...
        .card-title { color: #fff; font-size: 17px; font-weight: 600; }
        .card-sub   { color: var(--text-muted); font-size: 13px; margin-top: 3px; }

        .file-meta {
            display: flex; flex-direction: column; gap: 7px;
            border-top: 1px dashed var(--border-color); padding-top: 14px; margin-top: auto;
        }
        ...
        .empty-state {
            ...
            border: 1px dashed var(--border-color); padding: 70px 20px;
...
<div class="container">
    ...
    <div class="header-box">
        <div>
            <h1 class="main-title">Inspecciones PPR Almacenamiento</h1>
            <div class="sub-title">Sede Operativa: [ <?= htmlspecialchars($sede) ?> ] &nbsp;|&nbsp; OPE-AM-FO-003 v3 — Registros por mes</div>
        </div>
    ...
    <div class="stats-banner">
        ...
        <div class="stat-card">
            <div class="stat-val"><?= $total_reg_global ?></div>
    ...
    <div class="search-bar">
        <input type="text" class="search-input" id="searchInput"
               placeholder="Buscar por período, zona o usuario..."
               oninput="filtrarTarjetas(this.value)">
        <a href="inspeccion_almacenamiento.html" class="btn-new">
    ...
    <div class="grid" id="gridContainer">
        <?php if (empty($archivos)): ?>
            <div class="empty-state">
                <h3>Sin Registros</h3>
                <p>Aún no hay inspecciones de almacenamiento para la sede <strong><?= htmlspecialchars($sede) ?></strong>.</p>
                <a href="inspeccion_almacenamiento.html">+ Crear Primera Inspección</a>
            </div>
        <?php else: ?>
            ...
            ?>
                <a href="visor_inspeccion_almacenamiento.php?file=<?= urlencode($doc['filename']) ?>"
                   class="file-card"
                   data-search="<?= htmlspecialchars(strtolower($doc['periodo'] . ' ' . $doc['zona'] . ' ' . $doc['ultimo_usuario'])) ?>">
                    ...
                    <div>
                        <div class="card-title">Almacenamiento — <?= htmlspecialchars($doc['periodo']) ?></div>
                        <div class="card-sub">Zona: <?= htmlspecialchars($doc['zona']) ?></div>
                    </div>
...
<script>
    function filtrarTarjetas(query) {
        ...
        document.querySelectorAll('.file-card').forEach(card => {
            card.classList.toggle('hidden-by-search', q !== '' && !card.dataset.search.includes(q));
        });
    }
</script>
</body>
