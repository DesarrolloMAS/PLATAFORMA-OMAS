...
if (empty($input_array['responsable_nombre'])) {
    http_response_code(400);
    echo json_encode(['status' => 'error', 'message' => 'El nombre del responsable es obligatorio.']);
    exit;
}
...
$nuevo_registro = [
    'id_registro' => uniqid('INGGESTPROD_'),
...
$base_dir     = "../../archivos/generados/inspeccion_gestion_produccion_zs/";
$sede_dir     = $base_dir . preg_replace('/[^A-Za-z0-9_-]/', '', $sede) . "/";
$archivo_json = $sede_dir . "INGGESTPROD_" . $mes_actual . ".json";
...
if (@file_put_contents($archivo_json, json_encode($datos_existentes, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE))) {
    echo json_encode([
        'status'  => 'success',
        'message' => 'Inspección de gestión de la producción registrada correctamente.',
