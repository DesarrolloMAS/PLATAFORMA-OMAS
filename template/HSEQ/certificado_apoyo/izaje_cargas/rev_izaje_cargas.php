...
$target_dir   = "../../../../archivos/generados/cert_apoyo_izaje/" . $sede_saneada . "/";
...
if (file_exists($target_dir)) {
    foreach (scandir($target_dir) as $file) {
        ...
        $archivos[] = [
            'filename'       => $file,
            'periodo'        => str_replace(['IZAJ_', '.json'], '', $file),
            'registros'      => $total,
...
<head>
    ...
    <title>Revisiones — Izaje de Cargas</title>
    ...
    <style>
        :root {
            ...
            --success:#10B981; --izaje:#8B5CF6; --izaje-glow:rgba(139,92,246,0.4);
            --r-md:8px; --r-sm:4px;
        }
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .header-box {
            ...
            border-left:4px solid var(--izaje); padding:30px; border-radius:var(--r-md);
        ...
        .header-box::before {
            ...
            background:var(--izaje); color:#fff; font-family:'Space Mono',monospace;
            font-size:10px; font-weight:700; padding:4px 12px; border-radius:var(--r-sm);
            box-shadow:0 0 10px var(--izaje-glow);
        }
        ...
        .btn-back:hover { border-color:var(--izaje); color:var(--izaje); background:rgba(139,92,246,0.05); }
        ...
        .stat-val   { font-size:28px; font-weight:700; color:var(--izaje); font-family:'Space Mono',monospace; }
        ...
        .search-input:focus { outline:none; border-color:var(--izaje); box-shadow:0 0 0 3px rgba(139,92,246,0.1); }
        .search-input::placeholder { color:var(--text-muted); }
        .btn-new {
            background:var(--izaje); color:#fff; border:none; padding:12px 20px; border-radius:var(--r-sm);
            font-family:'Space Mono',monospace; font-size:12px; font-weight:700; text-decoration:none;
            text-transform:uppercase; white-space:nowrap; transition:all 0.3s;
            box-shadow:0 0 15px var(--izaje-glow); display:flex; align-items:center; gap:8px;
        }
        ...
        .file-card {
            ...
            padding:25px; display:flex; flex-direction:column; gap:14px; transition:all 0.3s;
            text-decoration:none; position:relative; overflow:hidden;
        }
        .file-card::after {
            content:''; position:absolute; bottom:0; left:0; right:0; height:3px;
            background:linear-gradient(90deg, var(--izaje), transparent); opacity:0; transition:opacity 0.3s;
        }
        .file-card:hover { border-color:rgba(139,92,246,0.4); transform:translateY(-5px); box-shadow:0 10px 25px rgba(139,92,246,0.08); }
        .file-card:hover::after { opacity:1; }
        .file-card.hidden-by-search { display:none; }
        .card-header { display:flex; justify-content:space-between; align-items:flex-start; gap:10px; }
        .periodo-badge {
            background:rgba(139,92,246,0.1); color:var(--izaje); border:1px solid rgba(139,92,246,0.25);
        ...
        .btn-view {
            background:transparent; color:var(--izaje); border:1px solid var(--izaje); padding:10px;
            border-radius:var(--r-sm); text-align:center; font-family:'Space Mono',monospace;
            font-weight:700; font-size:12px; text-transform:uppercase; transition:all 0.3s;
        }
        .file-card:hover .btn-view { background:var(--izaje); color:#fff; box-shadow:0 0 15px var(--izaje-glow); }
        .empty-state {
        ...
        .empty-state a  {
            background:var(--izaje); color:#fff; padding:12px 24px; border-radius:var(--r-sm);
...
<div class="container">
    <div class="header-box">
        <div>
            <h1 class="main-title">🏗️ Izaje de Cargas — Certificados</h1>
    ...
    <div class="grid">
        ...
        <?php else: ?>
            <?php foreach ($archivos as $doc): ?>
                <a href="visor_izaje_cargas.php?file=<?= urlencode($doc['filename']) ?>"
