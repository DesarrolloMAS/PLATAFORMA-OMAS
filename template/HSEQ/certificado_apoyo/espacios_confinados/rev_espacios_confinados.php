...
$target_dir   = "../../../../archivos/generados/cert_apoyo_espacios_conf/" . $sede_saneada . "/";
...
if (file_exists($target_dir)) {
    foreach (scandir($target_dir) as $file) {
        ...
        $archivos[] = [
            'filename'       => $file,
            'periodo'        => str_replace(['ECC_', '.json'], '', $file),
            'registros'      => $total,
...
<head>
    ...
    <title>Revisiones — Espacios Confinados</title>
    ...
    <style>
        :root {
            ...
            --success: #10B981; --warning: #FFB000; --r-md: 8px; --r-sm: 4px;
        }
        * { margin:0; padding:0; box-sizing:border-box; }
        body {
            ...
            background-image:
                radial-gradient(circle at top right, rgba(255,176,0,0.05), transparent 40%),
        ...
        .header-box {
            ...
            border-left:4px solid var(--warning); padding:30px; border-radius:var(--r-md);
        ...
        .header-box::before {
            ...
            background:var(--warning); color:var(--bg-color);
            font-family:'Space Mono',monospace; font-size:10px; font-weight:700;
            padding:4px 12px; border-radius:var(--r-sm); box-shadow:0 0 10px rgba(255,176,0,0.4);
        }
        ...
        .stat-val   { font-size:28px; font-weight:700; color:var(--warning); font-family:'Space Mono',monospace; }
        ...
        .btn-new {
            ...
            transition:all 0.3s; box-shadow:0 0 15px rgba(255,176,0,0.4);
        ...
        .file-card::after {
            ...
            background:linear-gradient(90deg, var(--warning), transparent); opacity:0; transition:opacity 0.3s;
        }
        .file-card:hover { border-color:rgba(255,176,0,0.4); transform:translateY(-5px); box-shadow:0 10px 25px rgba(255,176,0,0.08); }
        .file-card:hover::after { opacity:1; }
        .file-card.hidden-by-search { display:none; }

        .card-header { display:flex; justify-content:space-between; align-items:flex-start; gap:10px; }
        .periodo-badge {
            background:rgba(255,176,0,0.1); color:var(--warning);
            border:1px solid rgba(255,176,0,0.25); padding:4px 10px;
        ...
        .file-card:hover .btn-view { background:var(--warning); color:var(--bg-color); box-shadow:0 0 15px rgba(255,176,0,0.4); }
        ...
        .empty-state a  {
            background:var(--warning); color:var(--bg-color); padding:12px 24px;
...
<div class="container">
    ...
    <div class="header-box">
        <div>
            <h1 class="main-title">🛢️ Espacios Confinados — Certificados</h1>
    ...
    <div class="grid" id="gridContainer">
        ...
        <?php else: ?>
            <?php foreach ($archivos as $doc): ?>
                <a href="visor_espacios_confinados.php?file=<?= urlencode($doc['filename']) ?>"
                   class="file-card"
