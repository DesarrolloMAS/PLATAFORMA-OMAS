...
$target_dir   = "../../../../archivos/generados/cert_apoyo_electrico/" . $sede_saneada . "/";
...
if (file_exists($target_dir)) {
    foreach (scandir($target_dir) as $file) {
        ...
        $archivos[] = [
            'filename'       => $file,
            'periodo'        => str_replace(['ELEC_', '.json'], '', $file),
            'registros'      => $total,
...
<head>
    ...
    <title>Revisiones — Trabajo Riesgo Eléctrico</title>
    ...
    <style>
        :root {
            ...
            --success: #10B981; --elec: #EAB308; --elec-glow: rgba(234,179,8,0.4);
            --r-md: 8px; --r-sm: 4px;
        }
        * { margin:0; padding:0; box-sizing:border-box; }
        body {
            ...
            background-image:
                radial-gradient(circle at top right, rgba(234,179,8,0.05), transparent 40%),
        ...
        .header-box {
            ...
            border-left:4px solid var(--elec); padding:30px; border-radius:var(--r-md);
        ...
        .header-box::before {
            ...
            background:var(--elec); color:var(--bg-color);
            font-family:'Space Mono',monospace; font-size:10px; font-weight:700;
            padding:4px 12px; border-radius:var(--r-sm); box-shadow:0 0 10px var(--elec-glow);
        }
        ...
        .btn-back:hover { border-color:var(--elec); color:var(--elec); background:rgba(234,179,8,0.05); }

        .stats-banner { display:flex; gap:20px; margin-bottom:30px; flex-wrap:wrap; }
        .stat-card {
            background:var(--panel-bg); border:1px solid var(--border-color);
            padding:20px 28px; border-radius:var(--r-md); flex:1; min-width:160px;
            display:flex; flex-direction:column; gap:4px;
        }
        .stat-val   { font-size:28px; font-weight:700; color:var(--elec); font-family:'Space Mono',monospace; }
        ...
        .search-input:focus { outline:none; border-color:var(--elec); box-shadow:0 0 0 3px rgba(234,179,8,0.1); }
        .search-input::placeholder { color:var(--text-muted); }

        .btn-new {
            background:var(--elec); color:var(--bg-color); border:none;
            padding:12px 20px; border-radius:var(--r-sm);
            font-family:'Space Mono',monospace; font-size:12px; font-weight:700;
            text-decoration:none; text-transform:uppercase; white-space:nowrap;
            transition:all 0.3s; box-shadow:0 0 15px var(--elec-glow);
        ...
        .file-card::after {
            ...
            background:linear-gradient(90deg, var(--elec), transparent); opacity:0; transition:opacity 0.3s;
        }
        .file-card:hover { border-color:rgba(234,179,8,0.4); transform:translateY(-5px); box-shadow:0 10px 25px rgba(234,179,8,0.08); }
        .file-card:hover::after { opacity:1; }
        .file-card.hidden-by-search { display:none; }

        .card-header { display:flex; justify-content:space-between; align-items:flex-start; gap:10px; }
        .periodo-badge {
            background:rgba(234,179,8,0.1); color:var(--elec);
            border:1px solid rgba(234,179,8,0.25); padding:4px 10px;
            border-radius:var(--r-sm); font-family:'Space Mono',monospace; font-size:12px; font-weight:700;
        }
        .count-badge {
            background:rgba(16,185,129,0.1); color:var(--success);
            border:1px solid rgba(16,185,129,0.25); padding:4px 10px;
            border-radius:var(--r-sm); font-family:'Space Mono',monospace; font-size:11px; font-weight:700;
        }
        ...
        .btn-view {
            background:transparent; color:var(--elec); border:1px solid var(--elec);
        ...
        .file-card:hover .btn-view { background:var(--elec); color:var(--bg-color); box-shadow:0 0 15px var(--elec-glow); }
        ...
        .empty-state a  {
            background:var(--elec); color:var(--bg-color); padding:12px 24px;
...
<div class="container">
    ...
    <div class="header-box">
        <div>
            <h1 class="main-title">⚡ Riesgo Eléctrico — Certificados</h1>
    ...
    <div class="stats-banner">
        <div class="stat-card">
            <div class="stat-val"><?= $total_periodos ?></div>
            <div class="stat-label">Períodos</div>
        </div>
        <div class="stat-card">
            <div class="stat-val"><?= $total_registros ?></div>
            <div class="stat-label">Certificados Totales</div>
        </div>
    </div>

    <div class="search-bar">
        <input type="text" class="search-input" id="searchInput"
               placeholder="Buscar período (ej: 2026-06)..."
    ...
    <div class="grid" id="gridContainer">
        ...
        <?php else: ?>
            <?php foreach ($archivos as $doc): ?>
                <a href="visor_riesgo_electrico.php?file=<?= urlencode($doc['filename']) ?>"
                   class="file-card"
