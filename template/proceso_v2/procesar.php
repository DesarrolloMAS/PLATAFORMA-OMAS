...
if (!$input_array || empty($input_array['fecha']) || empty($input_array['referencia_producto'])) {
...
$mes_registro = date('Y-m', strtotime($fecha)) ?: date('Y-m');

$nuevo_registro = [
    'id_registro'  => uniqid('PROC_'),
...
$base_dir = "../../archivos/generados/proceso_v2/";
$sede_dir = $base_dir . preg_replace('/[^A-Za-z0-9_-]/', '', $sede) . "/";
$archivo_json = $sede_dir . "PROCESO_MOLIENDA_" . $mes_registro . ".json";
...
if (@file_put_contents($archivo_json, json_encode($datos_existentes, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE))) {
    echo json_encode(['status' => 'success', 'message' => 'Proceso de molienda registrado correctamente.', 'id' => $nuevo_registro['id_registro']]);
} else {
    $err = error_get_last();
