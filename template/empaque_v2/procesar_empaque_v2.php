...
if (!$input_array || (empty($input_array['producto_envasar']) && empty($input_array['lote_empaque']))) {
...
// Ruta de almacenamiento estructurado por LOTE DE EMPAQUE (MP-xxxxx) — unidad de trazabilidad
$lote_empaque = trim($input_array['lote_empaque'] ?? '');
if (empty($lote_empaque)) {
    $lote_empaque = 'N-A';
}
$lote_saneado = preg_replace('/[^A-Za-z0-9_-]/', '_', $lote_empaque);
$base_dir = "../../archivos/generados/empaque_v2/";
$sede_dir = $base_dir . preg_replace('/[^A-Za-z0-9_-]/', '', $sede) . "/";
$archivo_json = $sede_dir . "EMPAQUE_LOTE_" . $lote_saneado . ".json";
...
if (@file_put_contents($archivo_json, json_encode($datos_existentes, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE))) {
    echo json_encode(['status' => 'success', 'message' => 'Datos de empaque procesados en JSON correctamente.', 'id' => $nuevo_registro['id_registro']]);
} else {
    $err = error_get_last();
