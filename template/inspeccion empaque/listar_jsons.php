...
if (file_exists($target_dir)) {
    ...
    foreach ($files as $file) {
        ...
        $last_referencia  = $ultimo_registro['datos']['referencia_empaque']  ?? '—';
        $last_proveedor   = $ultimo_registro['datos']['proveedor']            ?? '—';
        $last_porcentaje  = $ultimo_registro['datos']['porcentaje_cumplimiento'] ?? null;
        $last_usuario     = $ultimo_registro['usuario_sys']                  ?? '—';

        // Extraer periodo del nombre de archivo: INSPECCION_YYYY-MM.json
