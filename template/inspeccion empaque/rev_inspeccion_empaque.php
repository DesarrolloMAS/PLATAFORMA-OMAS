...
$target_dir = "../../archivos/generados/inspeccion_empaque/" . $sede_saneada . "/";
...
if (file_exists($target_dir)) {
    ...
    foreach ($files as $file) {
        ...
        $last_referencia = $ultimo['datos']['referencia_empaque']       ?? 'Sin referencia';
        $last_proveedor  = $ultimo['datos']['proveedor']                ?? '—';
        ...
        $periodo = str_replace(['INSPECCION_', '.json'], '', $file);
...
// Promedio de porcentaje de cumplimiento del último registro de cada archivo
...
<head>
    ...
    <title>Revisiones - Inspección de Empaque</title>
...
<div class="container">
    ...
    <div class="header-box">
        <div>
            <h1 class="main-title">Inspecciones de Empaque</h1>
            <div class="sub-title">Sede Operativa: [ <?= htmlspecialchars($sede) ?> ] &nbsp;|&nbsp; Registros por mes</div>
        </div>
        <a href="../menu_seccion_sur.html" class="btn-back">← Menú Sección Sur</a>
    </div>
    ...
    <div class="search-bar">
        <input type="text" class="search-input" id="searchInput"
               placeholder="Buscar por período, referencia o proveedor..."
               oninput="filtrarTarjetas(this.value)">
        <a href="inspeccion_empaque.html" class="btn-new">
    ...
    <div class="grid" id="gridContainer">
        <?php if (empty($archivos)): ?>
            <div class="empty-state">
                <h3>Sin Registros</h3>
                <p>Aún no hay inspecciones de empaque guardadas para la sede <strong><?= htmlspecialchars($sede) ?></strong>.</p>
                <a href="inspeccion_empaque.html">+ Crear Primera Inspección</a>
            </div>
        <?php else: ?>
            ...
            ?>
                <a href="visor_inspeccion_empaque.php?file=<?= urlencode($doc['filename']) ?>"
                   class="file-card"
                   data-search="<?= htmlspecialchars(strtolower($doc['periodo'] . ' ' . $doc['referencia'] . ' ' . $doc['proveedor'])) ?>">
                    ...
                    <div>
                        <div class="card-title"><?= htmlspecialchars($doc['referencia']) ?></div>
                        <div class="card-sub">Proveedor: <?= htmlspecialchars($doc['proveedor']) ?></div>
                    </div>
