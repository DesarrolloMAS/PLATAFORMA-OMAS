...
if (file_exists($target_dir)) {
    ...
    foreach ($files as $file) {
        ...
        $bodega_key    = $ultimo['bodega_key']    ?? preg_replace('/_\d{4}-\d{2}\.json$/', '', $file);
...
<head>
    ...
    <title>Revisiones - Inspección de Bodegas</title>
...
<div class="container">
    ...
    <div class="header-box">
        <div>
            <h1 class="main-title">Inspecciones de Bodegas</h1>
            <div class="sub-title">Sede Operativa: [ <?= htmlspecialchars($sede) ?> ] &nbsp;|&nbsp; Registros por bodega/mes</div>
        </div>
        <a href="menu_bodegas_v2.php" class="btn-back">← Menú Bodegas</a>
    </div>

    <div class="stats-banner">
        <div class="stat-card">
            <div class="stat-val"><?= $total_archivos ?></div>
            <div class="stat-label">Archivos Bodega/Mes</div>
        </div>
        <div class="stat-card">
    ...
    <div class="search-bar">
        <input type="text" class="search-input" id="searchInput"
               placeholder="Buscar por bodega o período..."
               oninput="filtrarTarjetas(this.value)">
        <a href="menu_bodegas_v2.php" class="btn-new">
    ...
    <div class="grid" id="gridContainer">
        <?php if (empty($archivos)): ?>
            <div class="empty-state">
                <h3>Sin Registros</h3>
                <p>Aún no hay inspecciones de bodegas guardadas para la sede <strong><?= htmlspecialchars($sede) ?></strong>.</p>
                <a href="menu_bodegas_v2.php">+ Crear Primera Inspección</a>
            </div>
        <?php else: ?>
            ...
            ?>
                <a href="visor_bodegas_v2.php?file=<?= urlencode($doc['filename']) ?>"
                   class="file-card"
                   data-search="<?= htmlspecialchars(strtolower($doc['bodega_nombre'] . ' ' . $doc['periodo'])) ?>">
                    ...
                    <div>
                        <div class="card-title"><?= htmlspecialchars($doc['bodega_nombre']) ?></div>
                        <div class="card-sub">Clave: <?= htmlspecialchars($doc['bodega_key']) ?></div>
                    </div>

                    <div class="file-meta">
                        <div class="meta-line"><span>Inspecciones guardadas</span><span class="meta-val"><?= $doc['registros'] ?> registro(s)</span></div>
                        <div class="meta-line"><span>Última inspección</span><span class="meta-val"><?= htmlspecialchars($doc['ultima_fecha']) ?></span></div>
