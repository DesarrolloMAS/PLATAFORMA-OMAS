...
if (!$input_array || empty($input_array['fecha']) || empty($input_array['proveedor'])) {
    http_response_code(400);
    echo json_encode(['status' => 'error', 'message' => 'Faltan datos requeridos (fecha o proveedor).']);
    exit;
}

if (empty($input_array['insumos']) || !is_array($input_array['insumos'])) {
    http_response_code(400);
    echo json_encode(['status' => 'error', 'message' => 'Debe registrar al menos un insumo recibido.']);
    exit;
}
...
$base_dir = "../../archivos/generados/recepcion_insumos/";
$sede_dir = $base_dir . preg_replace('/[^A-Za-z0-9_-]/', '', $sede) . "/";
$archivo_json = $sede_dir . "RECEPCION_" . $mes_actual . ".json";
...
if (@file_put_contents($archivo_json, json_encode($datos_existentes, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE))) {
    echo json_encode(['status' => 'success', 'message' => 'Recepción de insumos procesada en JSON correctamente.', 'id' => $nuevo_registro['id_registro']]);
} else {
    $err = error_get_last();
