...
class CustomPDF extends TCPDF {
    public function Header() {
        if ($this->getPage() == 1) {
        $logo = '/var/www/img/logo_empresa.jpeg'; 
        $fecha = date('d/m/Y');
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $ruta_maquina = "/var/www/fmt/archivos/generados/verificaciones/camionera/{$codigo_maquina}-{$nombre_maquina}/";

    // Crear carpeta si no existe
    if (!file_exists($ruta_maquina)) mkdir($ruta_maquina, 0777, true);

    // Nombre base de archivos
    $nombre_archivo = "{$nombre_maquina}_{$codigo_maquina}_{$fecha_actual}_{$fecha_actual}";
    ...
    $directorio_imagenes = '/var/www//img/MAQUINAS/Camionera/';
    $imagen_path = '/var/www//img/default.jpeg';
