...
<head>
    ...
    <title>Revisión de Molienda - ZONA <?= $sede ?></title>
...
<div class="header">
    <h1>Galería de Revisiones: Molienda <?= $sede ?></h1>
    <a href="../redireccion.php" class="back-link">&lt; VOLVER</a>
</div>

<div class="container">
    <div class="month-selector" id="monthSelector">
        <!-- Monthly files will load here -->
    </div>
...
<script>
    async function init() {
        ...
        // Load most recent month automatically
    ...
    async function loadMonth(filePath, selectedCard) {
        ...
        const sede = '<?= $sede ?>';
        const resp = await fetch(`../../archivos/generados/molienda/${sede}/${filePath}`);
        ...
        let html = `
            <table class="records-table">
                <thead>
                    <tr>
                        <th>Fecha y Hora</th>
                        <th>Enviado por (Creador)</th>
                        <th>Almacenista (Receptor)</th>
        ...
        records.reverse().forEach((rec, idx) => {
            // Calculamos el nombre del archivo JSON original para referencia
            const archivoOrigen = filePath;
            html += `
                <tr onclick="window.location.href='visor_molienda.php?file=${filePath}&id=${rec.id}'">
                    <td>${rec.fecha} | ${rec.hora}</td>
                    <td style="color: var(--accent); font-weight: bold;">${rec.responsable}</td>
                    <td>${rec.almacenista}</td>
                    <td style="font-family: monospace; font-size: 11px; color: var(--text-muted);">
                       ID: ${rec.id.substring(0,6).toUpperCase()}
                    </td>
                    <td><a href="visor_molienda.php?file=${filePath}&id=${rec.id}" class="btn-view">VER REPORTE</a></td>
                </tr>
            `;
        });
