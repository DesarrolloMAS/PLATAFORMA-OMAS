...
<head>
    ...
    <title>REVISIONES - INSPECCIÓN DE INSUMOS</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .pct-badge { padding: 3px 10px; border-radius: var(--r-sm); font-family: 'Space Mono', monospace; font-size: 12px; font-weight: 700; white-space: nowrap; }
        .pct-badge.good   { background: rgba(16,185,129,0.15); color: var(--success); border: 1px solid rgba(16,185,129,0.3); }
        .pct-badge.warn   { background: rgba(255,176,0,0.15);  color: var(--warning); border: 1px solid rgba(255,176,0,0.3); }
        .pct-badge.danger { background: rgba(255,51,102,0.15); color: var(--danger);  border: 1px solid rgba(255,51,102,0.3); }
...
<div class="container">
    ...
    <div class="header-box">
        <div>
            <h1 class="main-title">Revisiones — Inspección de Insumos</h1>
            <div class="sub-title">Historial de registros por sede · <?php echo htmlspecialchars($_SESSION['sede'] ?? ''); ?></div>
        </div>
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
            <a href="index.html" class="btn-new">+ NUEVA INSPECCIÓN</a>
    ...
    <div class="filtros-card">
        ...
        <div class="filtro-group">
            ...
            <input type="text" id="filtroBuscar" class="filtro-control" placeholder="Planta, inspector, usuario…" style="min-width:220px;">
        </div>
    </div>
    ...
    <div class="system-status">
        ...
        SISTEMA JSON INTERCONECTADO · INSPECCIÓN DE INSUMOS
    </div>
</div>
...
<script>
    ...
    function renderTabla(registros) {
        ...
        registros.forEach(reg => {
            ...
            const pct = reg.promedio;
            const pctTxt = (pct !== null && pct !== undefined) ? pct.toFixed(1) + '%' : 'N/D';

            tr.innerHTML = `
                <td><span class="fecha-chip">${fechaFmt}</span></td>
                <td>${escHtml(reg.planta || '—')}</td>
                <td>${escHtml(reg.inspeccionado_por || '—')}</td>
                <td>${reg.num_items ?? 0}</td>
                <td><span class="pct-badge ${pctClass(pct)}">${pctTxt}</span></td>
                <td><span class="ts-text">${tsFmt}</span></td>
                <td>
                    <a href="visor_inspeccion_insumos.php?id=${encodeURIComponent(reg.id)}&file=${encodeURIComponent(reg.archivo)}"
    ...
    function aplicarFiltros() {
        ...
        const filtrado = todosLosRegistros.filter(reg => {
            ...
            if (buscar) {
                const haystack = [reg.planta, reg.inspeccionado_por, reg.verificado_por, reg.usuario].join(' ').toLowerCase();
