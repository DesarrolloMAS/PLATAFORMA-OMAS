...
if (file_exists($target_dir)) {
    ...
    foreach ($files as $file) {
        ...
        $last_referencia = $ultimo['datos']['referencia']   ?? 'Sin referencia';
        $last_lote       = $ultimo['datos']['lote']         ?? '—';
        $last_total      = isset($ultimo['datos']['total'])
                           ? floatval($ultimo['datos']['total'])
                           : null;
...
<head>
    ...
    <title>Revisiones - Control de Mejorante</title>
...
<div class="container">
    ...
    <div class="header-box">
        <div>
            <h1 class="main-title">Control de Mejorante</h1>
            <div class="sub-title">Sede Operativa: [ <?= htmlspecialchars($sede) ?> ] &nbsp;|&nbsp; Registros por mes</div>
        </div>
        <a href="../revisiones_almacen.html" class="btn-back">← Menú Revisiones Almacén</a>
    </div>
    ...
    <div class="stats-banner">
        ...
        <div class="stat-card">
            ...
            <div class="stat-label">Controles Totales</div>
        </div>
    </div>

    <div class="search-bar">
        <input type="text" class="search-input" id="searchInput"
               placeholder="Buscar por período, referencia o lote..."
               oninput="filtrarTarjetas(this.value)">
        <a href="control_mejorante.html" class="btn-new">
    ...
    <div class="grid" id="gridContainer">
        <?php if (empty($archivos)): ?>
            <div class="empty-state">
                <h3>Sin Registros</h3>
                <p>Aún no hay formatos de control guardados para la sede <strong><?= htmlspecialchars($sede) ?></strong>.</p>
                <a href="control_mejorante.html">+ Crear Primer Control</a>
            </div>
        <?php else: ?>
            <?php foreach ($archivos as $doc): ?>
                <a href="visor_control_mejorante.php?file=<?= urlencode($doc['filename']) ?>"
                   class="file-card"
                   data-search="<?= htmlspecialchars(strtolower($doc['periodo'] . ' ' . $doc['referencia'] . ' ' . $doc['lote'])) ?>">
                    ...
                    <div>
                        ...
                        <div class="card-sub">Lote: <?= htmlspecialchars($doc['lote']) ?></div>
                    </div>
                    ...
                    <div class="btn-view">VER FORMATOS →</div>
                </a>
            <?php endforeach; ?>
        <?php endif; ?>
    </div>
