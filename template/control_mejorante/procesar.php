...
if (!$input_array || empty($input_array['referencia']) || empty($input_array['lote'])) {
...
$base_dir = "../../archivos/generados/control_mejorante/";
$sede_dir = $base_dir . preg_replace('/[^A-Za-z0-9_-]/', '', $sede) . "/";
$archivo_json = $sede_dir . "MEJORANTE_" . $mes_actual . ".json";
...
if (@file_put_contents($archivo_json, json_encode($datos_existentes, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE))) {
    echo json_encode(['status' => 'success', 'message' => 'Control de mejorante procesado en JSON correctamente.', 'id' => $nuevo_registro['id_registro']]);
} else {
    $err = error_get_last();
